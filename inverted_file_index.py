...
# Calculates the Inverted File Index for the given documents.
class MRInvertedFileIndex(MRJob):
    def steps(self):
        return [
    ...
    '''
    Yields [word, list(document name, cumulative_occurrences)] for each word after sorting the list in descendant
    order using the cumulative occurrences as criterion.
    '''
    ...
    def reducer_sort_inverted_file_index_for_word(self, word, doc_name_and_cumulative_occurrences):
        ...
        yield word, doc_name_and_cumulative_occurrences_list

if __name__ == '__main__':
    MRInvertedFileIndex.run()
