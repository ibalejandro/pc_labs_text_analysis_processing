...
# Calculates the Inverted File Index sorted by length for the given documents.
class MRInvertedFileIndexLengthSorted(MRJob):
    def steps(self):
        return [
            ...
            MRStep(reducer=self.reducer_sort_inverted_file_index_by_length)
        ]
    ...
    def reducer_sort_inverted_file_index_for_word(self, word, doc_name_and_cumulative_occurrences):
        ...
        # Uses the Inverted File Index length for each word as the criterion for the next reduction. That will mean to
        # sort the entire Inverted File Index in descendant order by the number of documents in which the word appears.
        criterion_for_next_reduction = len(doc_name_and_cumulative_occurrences_list)
    ...
    Index in descendant order using the number of documents in which the word appears as criterion.
    '''

    def reducer_sort_inverted_file_index_by_length(self, _, word_ifi_and_criterion):
        ...
        # Sorts the tuple list in descendant order using the number of documents in which the word appears as criterion.
...
if __name__ == '__main__':
    MRInvertedFileIndexLengthSorted.run()
